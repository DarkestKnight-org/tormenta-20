...
(function (global) {
  ...
  async function init() {
    ...
    client.auth.onAuthStateChange((_event, session) => {
      ...
      if (typeof SUPA.state.onAuthChange === 'function') {
        SUPA.state.onAuthChange(SUPA.state.user);
      }
    });
  ...
  function onAuthChange(callback) {
    SUPA.state.onAuthChange = typeof callback === 'function' ? callback : null;
  }
  ...
  async function signUp(email, password, displayName) {
    ...
    async function alterarNomeUsuario(nomeExibicao) {
        const client = ensureClient();
        ...
        if (error) throw error;
        SUPA.state.user = data?.user || SUPA.state.user;
        return data;
    }
  ...
  async function listarMinhasFichas() {
    const client = ensureClient();
    if (!SUPA.state.user) throw new Error('Usuário não autenticado.');

    const { data, error } = await client
      .from('characters')
      .select('id, mesa_id, nome, ficha_local_id, ficha_json, is_active, updated_at')
  ...
  async function buscarMesaPorId(mesaId) {
    const client = ensureClient();
    const { data, error } = await client
      .from('mesas')
      .select('id, nome, codigo_convite, master_user_id, created_at')
  ...
  async function excluirFichaPorLocalId(fichaLocalId) {
    const client = ensureClient();
    if (!SUPA.state.user) throw new Error('Usuário não autenticado.');
  ...
  async function desativarFicha({ mesaId, fichaLocalId }) {
    ...
    function agendarSyncAmeacaMestre({ mesaId, ameaca, wait = 700 }) {
        clearTimeout(SUPA.state.syncTimerAmeaca);
        SUPA.state.syncTimerAmeaca = setTimeout(async () => {
            try {
  ...
  function agendarSyncFichaAtiva({ mesaId, ficha, wait = 900 }) {
    clearTimeout(SUPA.state.syncTimerFicha);
    SUPA.state.syncTimerFicha = setTimeout(async () => {
      try {
  ...
  async function flushSyncFichaAtiva({ mesaId, ficha }) {
    clearTimeout(SUPA.state.syncTimerFicha);
    return syncFichaAtivaAgora({ mesaId, ficha });
  }

  async function uploadImagemPersonagem({ fichaId, tipo, blob, contentType }) {
    const client = ensureClient();
    if (!SUPA.state.user) throw new Error('Usuário não autenticado.');

    const tipoArquivo = tipo === 'avatar' ? 'avatar' : 'banner';
    const extensao = String(contentType || '').toLowerCase().includes('png') ? 'png' : 'jpg';
    const caminho = `${SUPA.state.user.id}/${String(fichaId)}/${tipoArquivo}.${extensao}`;
    ...
    return {
      path: caminho,
      url: data?.publicUrl ? `${data.publicUrl}?v=${Date.now()}` : ''
    };
  }

  async function excluirImagensPersonagem(paths) {
    const client = ensureClient();
    if (!SUPA.state.user) throw new Error('Usuário não autenticado.');
    ...
    const lista = (Array.isArray(paths) ? paths : [])
      ...
      .filter(Boolean)
      .filter(path => path.startsWith(`${SUPA.state.user.id}/`));
  ...
  global.T20Supabase = {
    ...
    alterarSenha,
    alterarNomeUsuario,
    uploadImagemPersonagem,
    excluirImagensPersonagem,
    salvarAmeacaMestre,
    removerAmeacaMestre,
  ...
  global.T20Supabase.ready = init().catch((err) => {
